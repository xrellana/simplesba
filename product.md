...
## 当前决策：暂缓开发（2026-09-24）

结论：按目前的需求，没有必要专门开发 simplesba。先试用现有方案，暂不启动后文的开发路线图。

主要理由：

* 核心需求“显示 Blizzard 推荐技能及对应快捷键”已有现成插件覆盖，例如 JustAC；避免 Single-Button Assistant 的 GCD 惩罚也不是本插件的独有优势。
* 自行开发需要持续处理游戏版本更新、宏、动作条翻页和键位兼容，维护成本高于一个简单 HUD 的表面工作量。
* 当前尚未明确现有方案无法满足的具体需求，因此缺少值得单独开发的实际收益。

下一步先使用 Assisted Highlight，并试用 JustAC 或 NextGCD。只有实际使用后发现明确、影响体验、且无法通过现有设置解决的问题，才重新评估开发；届时只围绕这些问题定义最小版本。若现有方案已够用，则继续搁置。

如果目的改为学习插件开发，可以作为练手项目，但应与实用需求分开评估。

下文保留为前期调研和候选设计，不代表已承诺实施的需求或开发计划；项目是否启动以本节为准。
